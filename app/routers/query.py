...
async def query(body: QueryRequest, user=Depends(get_current_user)):
    ...
    return StreamingResponse(
        _event_stream(graph, initial, config, thread_id),
        media_type="text/event-stream",
        headers={"X-Accel-Buffering": "no", "Cache-Control": "no-cache"},
    )

async def _event_stream(graph, initial, config, thread_id):
    try:
        async for event in graph.astream_events(initial, config=config, version="v2"):
            ...
            if etype == "on_chat_model_stream":
                ...
                "input","supervisor","safety_agent","rag_agent","data_agent","synthesizer"
            ):
...
def _trunc(s: str, n: int) -> str: return s[:n] + "…" if len(s) > n else s